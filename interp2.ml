...
let rec compile_expr expr =
  ...
  | Unit -> ["Push Unit"]
  | UOpr (Neg, x) ->
    compile_expr x @ ["Push -1"] @ ["Mul"]
  | UOpr (Not, x) ->
    compile_expr x @ ["Not"]
  | Let (name, rhs, expr) -> 
    (*["NAME: " ^ name ^ " RHS: " ^ string_of_expr rhs ^ " EXPR:" ^ string_of_expr expr ^ "DONEDEBUG SECTION"] @ *)
    compile_expr rhs @ ["Push " ^ name] @ ["Bind"] @ compile_expr expr
  | Fun (f, x, body) ->
    ["Push " ^ f] @ ["Fun Push " ^ x] @ ["Bind"] @ compile_expr body @ ["Swap"] @ ["Return"] @ ["End"]
  | App (func, arg) ->
    compile_expr func @ compile_expr arg @ ["Swap"] @ ["Call"] 
  | BOpr (Add, e1, e2) ->
    compile_expr e1 @ 
    compile_expr e2 @ 
    ["Add"]
  ...
  | BOpr (Mod, e1, e2) ->
    compile_expr e1 @ 
    compile_expr e2 @ 
    ["Swap"] @ ["Div"] @ compile_expr e2 @ ["Mul"] @ compile_expr e1 @ ["Sub"]
  | BOpr (Or, e1, e2) ->
    compile_expr e1 @ 
    compile_expr e2 @ 
    ["Or"]
  ...
  | BOpr (Eq, e1, e2) ->
    compile_expr e1 @ 
    compile_expr e2 @ 
    ["Swap"] @ ["Lt"] @ compile_expr e1 @ 
    compile_expr e2 @ ["Swap"; "Gt"; "Or"; "Not"]
  | BOpr (Gte, e1, e2) ->
    compile_expr e1 @ 
    compile_expr e2 @ 
    ["Swap"; "Lt"; "Not"]
  | BOpr (Lte, e1, e2) ->
    ...
    compile_expr e2 @ 
    ["Swap"; "Gt"; "Not"]
  | Ifte (cond, then_expr, else_expr) ->
      let compiled_cond = compile_expr cond in
      let compiled_then = String.concat "; " (compile_expr then_expr) in
      let compiled_else = String.concat "; " (compile_expr else_expr) in
      compiled_cond @ 
      ["If " ^ compiled_then] @
      ["Else " ^ compiled_else] @
      ["End"]
  | Trace m -> compile_expr m @ ["Trace"]
  | Seq (e1, e2) -> compile_expr e1 @ ["Pop"] @ compile_expr e2
  | Var s -> ["Push " ^ s ^ "; Lookup"]
  | _ -> raise SyntaxError
  let compile (s : string) : string =
    let expr = parse_prog s in
    let commands = compile_expr expr in
